...
FKXmlFile::FKXmlFile(const FString& InFile, EKConstructMethod::Type ConstructMethod)
	: RootNode(nullptr), bFileLoaded(false)
{
	LoadFile(InFile, ConstructMethod);
}

bool FKXmlFile::LoadFile(const FString& InFile, EKConstructMethod::Type ConstructMethod)
{
	...
	if(ConstructMethod == EKConstructMethod::ConstructFromFile)
	{
...
FString FKXmlFile::GetLastError() const
{
	return ErrorMessage;
}

void FKXmlFile::Clear()
{
	if(bFileLoaded)
	{
...
bool FKXmlFile::IsValid() const
{
	checkSlow(bFileLoaded ? RootNode != nullptr : RootNode == nullptr);
	return bFileLoaded;
}

const FKXmlNode* FKXmlFile::GetRootNode() const
{
	return RootNode;
}

FKXmlNode* FKXmlFile::GetRootNode()
{
	return RootNode;
}

bool FKXmlFile::Save(const FString& Path)
{
	FString Xml = TEXT("<?xml version=\"1.0\" encoding=\"UTF-8\"?>") LINE_TERMINATOR;

	const FKXmlNode* CurrentNode = GetRootNode();
...
void FKXmlFile::PreProcessInput(TArray<FString>& Input)
{
	// Note: This implementation is written simply and will not handle all cases.  It
	//       is made for the simple cases where FKXmlFile is to be used.
	// Assumptions/Misc:
...
void FKXmlFile::WhiteOut(TArray<FString>& Input, int32 LineStart, int32 LineEnd, int32 IndexStart, int32 IndexEnd)
{
...
void FKXmlFile::Tokenize(FStringView Input, TArray<FString>& Tokens)
{
...
TArray<FString> FKXmlFile::Tokenize(const TArray<FString>& Input)
{
...
void FKXmlFile::AddAttribute(const FString& InToken, TArray<FKXmlAttribute>& OutAttributes)
{
	int32 EqualsIdx;
	if(InToken.FindChar(TEXT('='), EqualsIdx))
	{
		...
		if(bQuotesRemoved)
		{
			...
			OutAttributes.Add(FKXmlAttribute(InToken.Left(EqualsIdx), Value));
		}
	}
}

FKXmlNode* FKXmlFile::CreateNodeRecursive(const TArray<FString>& Tokens, int32 StartIndex, int32* OutNextIndex)
{
	// Algorithm (Draft):
	...
	for(int32 i = StartIndex; i < Tokens.Num(); ++i)
	{
		...
		else if(bInTag)
		{
			...
			else
			{
				...
				{
					FKXmlNode* NewNode = new FKXmlNode();
					NewNode->Tag = Tag;
...
void FKXmlFile::HookUpNextPtrs(FKXmlNode* Node)
{
	if(Node == nullptr)
	{
		return;
...
void FKXmlFile::WriteNodeHierarchy(const FKXmlNode& Node, const FString& Indent, FString& Output)
{
	// Write the tag
	Output += Indent + FString::Printf(TEXT("<%s"), *Node.GetTag());
	for(const FKXmlAttribute& Attribute: Node.GetAttributes())
	{
	...
	const FKXmlNode* FirstChildNode = Node.GetFirstChildNode();
	...
	{
		Output += TEXT(">") LINE_TERMINATOR;
		for(const FKXmlNode* ChildNode = FirstChildNode; ChildNode != nullptr; ChildNode = ChildNode->GetNextNode())
		{
